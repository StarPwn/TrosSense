...
class FindAddress {
    ...
    private static int attemptMatch(String content, MatchResult houseNumberMatch) {
        ...
        while (true) {
            if (it2 < content.length()) {
                ...
                while (it2 < matcher.start()) {
                    int it3 = it2 + 1;
                    if (NL.indexOf(content.charAt(it2)) != -1) {
                        numLines++;
                    }
                    it2 = it3;
                }
                ...
                if (matchHouseNumber(content, it2) != null) {
                    ...
                    if (isValidLocationName(matcher.group(0))) {
                        ...
                        if (foundLocationName && wordCount > 4 && (stateMatch = matchState(content, it2)) != null) {
        ...
        return -(restartPos > 0 ? restartPos : it2);
    }
