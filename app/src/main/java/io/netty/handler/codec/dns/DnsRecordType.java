...
public class DnsRecordType implements Comparable<DnsRecordType> {
    ...
    public static final DnsRecordType TKEY = new DnsRecordType(bl.dk, "TKEY");
    public static final DnsRecordType TSIG = new DnsRecordType(250, "TSIG");
    public static final DnsRecordType IXFR = new DnsRecordType(bl.dm, "IXFR");
    public static final DnsRecordType AXFR = new DnsRecordType(bl.dn, "AXFR");
    ...
    static {
        DnsRecordType[] all = {A, NS, CNAME, SOA, PTR, MX, TXT, RP, AFSDB, SIG, KEY, AAAA, LOC, SRV, NAPTR, KX, CERT, DNAME, OPT, APL, DS, SSHFP, IPSECKEY, RRSIG, NSEC, DNSKEY, DHCID, NSEC3, NSEC3PARAM, TLSA, HIP, SPF, TKEY, TSIG, IXFR, AXFR, ANY, CAA, TA, DLV, SVCB, HTTPS};
        StringBuilder expected = new StringBuilder(512);
        expected.append(" (expected: ");
        for (DnsRecordType type : all) {
            BY_NAME.put(type.name(), type);
            BY_TYPE.put(type.intValue(), (int) type);
    ...
    public String toString() {
        ...
        if (text == null) {
            String text2 = this.name + '(' + intValue() + ')';
            this.text = text2;
            return text2;
        }
        return text;
