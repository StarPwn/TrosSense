...
public final class buffer implements BufferedSource {
    ...
    public long read(Buffer sink, long byteCount) {
        ...
        if (this.bufferField.size() == 0) {
            long read$iv = this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE);
            if (read$iv == -1) {
                return -1L;
            }
        }
    ...
    public boolean exhausted() {
        if (!this.closed) {
            return this.bufferField.exhausted() && this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE) == -1;
        }
        throw new IllegalStateException("closed".toString());
    }
    ...
    public boolean request(long byteCount) {
        ...
        while (this.bufferField.size() < byteCount) {
            if (this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE) == -1) {
                return false;
            }
        }
        return true;
    ...
    public int select(Options options) {
        ...
        } while (this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE) != -1);
        return -1;
    }
    ...
    public int read(ByteBuffer sink) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        if (this.bufferField.size() == 0) {
            long read = this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE);
            if (read == -1) {
                return -1;
            }
        }
    ...
    public int readUtf8CodePoint() {
        ...
        if ((b0$iv & bl.cW) == 192) {
            require(2L);
        } else if ((b0$iv & bl.db) == 224) {
            require(3L);
        } else if ((b0$iv & bl.dj) == 240) {
            require(4L);
        }
    ...
    public void skip(long byteCount) {
        ...
        while (byteCount$iv > 0) {
            if (this.bufferField.size() == 0 && this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE) == -1) {
                throw new EOFException();
            }
    ...
    public long indexOf(byte b, long fromIndex, long toIndex) {
        ...
        while (fromIndex$iv < toIndex) {
            ...
            if (lastBufferSize$iv >= toIndex || this.source.read(this.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE) == -1) {
                return -1L;
            }
        ...
        return new InputStream() { // from class: okio.RealBufferedSource$inputStream$1
            ...
            public int read() {
                ...
                if (this_$iv.bufferField.size() == 0) {
                    ...
                    long count = source.read(this_$iv2.bufferField, Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE);
                    if (count == -1) {
                        return -1;
                    }
                }
