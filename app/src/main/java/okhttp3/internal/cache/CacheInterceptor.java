...
public final class CacheInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (networkRequest == null && cacheResponse == null) {
            Response it2 = new Response.Builder().request(chain.request()).protocol(Protocol.HTTP_1_1).code(504).message("Unsatisfiable Request (only-if-cached)").body(Util.EMPTY_RESPONSE).sentRequestAtMillis(-1L).receivedResponseAtMillis(System.currentTimeMillis()).build();
            listener.satisfactionFailure(call, it2);
            return it2;
        }
        if (networkRequest == null) {
            Intrinsics.checkNotNull(cacheResponse);
            Response it3 = cacheResponse.newBuilder().cacheResponse(INSTANCE.stripBody(cacheResponse)).build();
            listener.cacheHit(call, it3);
            return it3;
        }
