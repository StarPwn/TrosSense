...
public final class FloatBigArrays {
    ...
    public static float[][] newBigArray(long length) {
        ...
        if (residual != 0) {
            for (int i = 0; i < baseLength - 1; i++) {
                base[i] = new float[BigArrays.SEGMENT_SIZE];
            }
            base[baseLength - 1] = new float[residual];
        } else {
            for (int i2 = 0; i2 < baseLength; i2++) {
                base[i2] = new float[BigArrays.SEGMENT_SIZE];
            }
        }
        return base;
    }
    ...
    */
    public static long med3(float[][] r5, long r6, long r8, long r10, it.unimi.dsi.fastutil.floats.FloatComparator r12) {
        /*
            float r0 = it.unimi.dsi.fastutil.BigArrays.get(r5, r6)
            float r1 = it.unimi.dsi.fastutil.BigArrays.get(r5, r8)
            int r0 = r12.compare(r0, r1)
            float r1 = it.unimi.dsi.fastutil.BigArrays.get(r5, r6)
            float r2 = it.unimi.dsi.fastutil.BigArrays.get(r5, r10)
            int r1 = r12.compare(r1, r2)
            float r2 = it.unimi.dsi.fastutil.BigArrays.get(r5, r8)
            float r3 = it.unimi.dsi.fastutil.BigArrays.get(r5, r10)
        ...
        L35:
            return r3
        */
        throw new UnsupportedOperationException("Method not decompiled: it.unimi.dsi.fastutil.floats.FloatBigArrays.med3(float[][], long, long, long, it.unimi.dsi.fastutil.floats.FloatComparator):long");
    }
    ...
    public static void quickSort(float[][] x, long from, long to, FloatComparator comp) {
        ...
        int comparison;
        int comparison2;
        FloatComparator floatComparator = comp;
        long len = to - from;
        if (len < 7) {
        ...
        long b2 = from;
        while (true) {
            if (b2 <= b && (comparison2 = floatComparator.compare(BigArrays.get(x, b2), v)) <= 0) {
                if (comparison2 == 0) {
                ...
                d = c3;
                c = b;
                while (c >= b2 && (comparison = floatComparator.compare(BigArrays.get(x, c), v)) >= 0) {
                    if (comparison == 0) {
    ...
    /*
        ...
        To view partially-correct code enable 'Show inconsistent code' option in preferences
    */
    public static long med3(float[][] r5, long r6, long r8, long r10) {
        /*
            float r0 = it.unimi.dsi.fastutil.BigArrays.get(r5, r6)
            float r1 = it.unimi.dsi.fastutil.BigArrays.get(r5, r8)
        ...
        L35:
            return r3
        */
        throw new UnsupportedOperationException("Method not decompiled: it.unimi.dsi.fastutil.floats.FloatBigArrays.med3(float[][], long, long, long):long");
    }
    ...
    public static void quickSort(float[][] x, long from, long to) {
        ...
        long b2 = from;
        while (true) {
            if (b2 <= b && (comparison2 = Float.compare(BigArrays.get(x, b2), v)) <= 0) {
                if (comparison2 == 0) {
                    BigArrays.swap(x, a, b2);
                    a++;
                }
                ...
                long len2 = d2;
                d = len;
                while (c >= b2 && (comparison = Float.compare(BigArrays.get(x, c), v)) >= 0) {
                    if (comparison == 0) {
                        BigArrays.swap(x, c, d);
                        d--;
                    }
    ...
    public static class ForkJoinQuickSort extends RecursiveAction {
        ...
        protected void compute() {
            ...
            long s = len / 8;
            long l2 = FloatBigArrays.med3(x, l, l + s, l + (s * 2));
            long m2 = FloatBigArrays.med3(x, m - s, m, m + s);
            long d2 = FloatBigArrays.med3(x, n - (2 * s), n - s, n);
            long m3 = FloatBigArrays.med3(x, l2, m2, d2);
            float v = BigArrays.get(x, m3);
            long a = this.from;
            ...
            long len2 = a;
            while (true) {
                if (len2 <= c2 && (comparison2 = Float.compare(BigArrays.get(x, len2), v)) <= 0) {
                    if (comparison2 == 0) {
                    ...
                    long n3 = d2;
                    d = n2;
                    while (c >= len2 && (comparison = Float.compare(BigArrays.get(x, c), v)) >= 0) {
                        if (comparison == 0) {
            ...
            FloatBigArrays.swap(x, this.from, len2 - s2, s2);
            long s3 = Math.min(d - c3, (this.to - d) - serialVersionUID);
            FloatBigArrays.swap(x, len2, this.to - s3, s3);
            long s4 = len2 - a2;
            long t = d - c3;
    ...
    public static class ForkJoinQuickSortComp extends RecursiveAction {
        ...
        protected void compute() {
            ...
            long s = len / 8;
            long l2 = FloatBigArrays.med3(x, l, l + s, l + (s * 2), this.comp);
            long m2 = FloatBigArrays.med3(x, m - s, m, m + s, this.comp);
            long n3 = FloatBigArrays.med3(x, n2 - (2 * s), n2 - s, n2, this.comp);
            float v = BigArrays.get(x, FloatBigArrays.med3(x, l2, m2, n3, this.comp));
            long a = this.from;
            ...
            long c6 = b;
            FloatBigArrays.swap(x, this.from, b - s2, s2);
            long s3 = Math.min(d - c5, (this.to - d) - serialVersionUID);
            FloatBigArrays.swap(x, c6, this.to - s3, s3);
            long s4 = c6 - a3;
            long t = d - c5;
            if (s4 > serialVersionUID && t > serialVersionUID) {
        ...
        while (lengthPos > 0) {
            ...
            if (length < 40) {
                ...
                while (true) {
                    ...
                    BigArrays.set(digit, i2, (byte) (((fixFloat(BigArrays.get(a, first + i2)) >>> shift2) & 255) ^ signMask));
                    maxLevel = maxLevel;
                    stackSize = stackSize;
                    i = i2;
                    level3 = level;
                ...
                while (i7 < end) {
                    ...
                    float t = BigArrays.get(a, i7 + first);
        ...
        while (offsetPos > 0) {
            ...
            if (length < 40) {
                ...
                while (true) {
                    ...
                    BigArrays.set(digit2, i3, (byte) (((fixFloat(BigArrays.get(k, first + i3)) >>> shift) & 255) ^ signMask));
                    i2 = i3;
                    layers2 = layers;
                    maxLevel2 = maxLevel;
                }
                ...
                while (i8 < end) {
                    float[][] k2 = k;
                    int shift2 = shift;
                    float t = BigArrays.get(a, i8 + first);
                    float u = BigArrays.get(b, i8 + first);
                    ...
                    while (true) {
                        ...
                        if (d > i8) {
                            ...
                            int zz2 = c;
                            int stackSize = zz;
                            float t2 = BigArrays.get(a, d + first);
                            BigArrays.set(a, d + first, z);
                            float z2 = u2;
    ...
    private static void insertionSortIndirect(long[][] perm, float[][] a, float[][] b, long from, long to) {
        ...
        while (true) {
            ...
            if (j < to) {
                ...
                while (true) {
                    if (Float.compare(BigArrays.get(a, t), BigArrays.get(a, u)) < 0 || (Float.compare(BigArrays.get(a, t), BigArrays.get(a, u)) == 0 && Float.compare(BigArrays.get(b, t), BigArrays.get(b, u)) < 0)) {
    ...
    public static void radixSortIndirect(long[][] perm, float[][] a, float[][] b, long from, long to, boolean stable) {
        ...
        while (stackPos > 0) {
            ...
            while (true) {
                ...
                int fixFloat = ((fixFloat(BigArrays.get(k, BigArrays.get(perm, i3))) >>> shift) & 255) ^ signMask;
                count3[fixFloat] = count3[fixFloat] + 1;
                level = level;
                i2 = i3;
            }
            int level2 = level;
            ...
            if (stable) {
                long i5 = first + length;
                while (true) {
                    long i6 = i5 - 1;
                    if (i5 == first) {
                        break;
                    }
                    int fixFloat2 = ((fixFloat(BigArrays.get(k, BigArrays.get(perm, i6))) >>> shift) & 255) ^ signMask;
                    long j3 = pos2[fixFloat2] - 1;
                    pos2[fixFloat2] = j3;
                    BigArrays.set(support2, j3, BigArrays.get(perm, i6));
                    k = k;
                    count3 = count3;
                    pos2 = pos2;
                    i5 = i6;
                ...
                long p2 = first;
                int i7 = 0;
                for (int layers3 = stackSize4; i7 < layers3; layers3 = 256) {
                    if (level2 < 7) {
                        long[] count5 = count4;
                        if (count5[i7] <= 1) {
                            count2 = count5;
                            maxLevel2 = maxLevel3;
                        } else if (count5[i7] < RealWebSocket.DEFAULT_MINIMUM_DEFLATE_SIZE) {
                            count2 = count5;
                            maxLevel2 = maxLevel3;
                            insertionSortIndirect(perm, a, b, p2, p2 + count5[i7]);
                        } else {
                            count2 = count5;
                            maxLevel2 = maxLevel3;
                            offsetStack[stackPos2] = p2;
                            lengthStack[stackPos2] = count2[i7];
                ...
                long i8 = first;
                while (i8 <= end) {
                    long t = BigArrays.get(perm, i8);
                    int c2 = ((fixFloat(BigArrays.get(k, t)) >>> shift) & 255) ^ signMask;
                    if (i8 >= end) {
                        ...
                        while (true) {
                            ...
                            c2 = ((fixFloat(BigArrays.get(k, t)) >>> shift) & 255) ^ signMask;
                            stackSize3 = stackSize3;
                        }
                        stackSize2 = stackSize3;
                        c = c2;
                        BigArrays.set(perm, i8, t);
                    }
                    if (level2 >= 7 || count[c] <= 1) {
                        ...
                        z = 7;
                        i = i8;
                        insertionSortIndirect(perm, a, b, i8, i8 + count[c]);
                    } else {
                        z = 7;
                        i = i8;
