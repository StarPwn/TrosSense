...
public final class ValueFactory {
    ...
    public static class MapBuilder {
        ...
        public MapBuilder putAll(Map<? extends Value, ? extends Value> map) {
            Iterator<Map.Entry<? extends Value, ? extends Value>> it2 = map.entrySet().iterator();
            while (it2.hasNext()) {
                put(it2.next());
            }
            return this;
    ...
    public static ImmutableTimestampValue newTimestamp(long j) {
        return newTimestamp(Instant.ofEpochMilli(j));
    }

    public static ImmutableTimestampValue newTimestamp(long j, int i) {
        return newTimestamp(Instant.ofEpochSecond(j, i));
    }
}
