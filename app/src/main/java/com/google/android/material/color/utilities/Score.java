...
public final class Score {
    ...
    public static List<Integer> score(Map<Integer, Integer> colorsToPopulation, int maxColorCount, int fallbackColorArgb, boolean filter) {
        ...
        for (Hct hct2 : colorsHct) {
            ...
            if (!filter || (hct2.getChroma() >= 5.0d && proportion2 > CUTOFF_EXCITED_PROPORTION)) {
        ...
        for (int differenceDegrees = 90; differenceDegrees >= 15; differenceDegrees--) {
            chosenColors.clear();
            Iterator<ScoredHCT> it2 = scoredHcts.iterator();
            while (it2.hasNext()) {
                Hct hct3 = it2.next().hct;
                boolean hasDuplicateHue = false;
                Iterator<Hct> it3 = chosenColors.iterator();
                while (true) {
                    if (!it3.hasNext()) {
                        break;
                    }
                    Hct chosenHct = it3.next();
