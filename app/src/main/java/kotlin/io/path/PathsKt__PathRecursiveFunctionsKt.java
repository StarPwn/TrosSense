            ...
            if (!targetExistsAndNotSymlink || !Files.isSameFile($this$copyToRecursively, target)) {
                if (Intrinsics.areEqual($this$copyToRecursively.getFileSystem(), target.getFileSystem())) {
                    if (targetExistsAndNotSymlink) {
                        ...
                        Path it2 = target.getParent();
                        if (it2 != null && Files.exists(it2, (LinkOption[]) Arrays.copyOf(new LinkOption[0], 0)) && it2.toRealPath(new LinkOption[0]).startsWith($this$copyToRecursively.toRealPath(new LinkOption[0]))) {
                            isSubdirectory = true;
                        }
                    }
                }
    ...
    public static final void deleteRecursively(Path $this$deleteRecursively) {
        ...
        if (!((Collection) suppressedExceptions).isEmpty()) {
            ...
            for (Object element$iv : $this$forEach$iv) {
                Exception it2 = (Exception) element$iv;
                ExceptionsKt.addSuppressed($this$deleteRecursively_u24lambda_u242, it2);
            }
