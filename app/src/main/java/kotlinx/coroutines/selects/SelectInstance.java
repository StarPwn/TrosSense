...
public final class SelectInstance<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, kotlinx.coroutines.selects.SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
        ...
        while (true) {
            ...
            if (result$iv == SelectKt.access$getUNDECIDED$p()) {
                ...
                if (result$iv == update$iv2) {
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_result$FU, this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.access$getRESUMED$p())) {
                        if (Result.m286isFailureimpl(result)) {
                            Continuation $this$resumeWithStackTrace$iv = this.uCont;
                            Throwable exception$iv = Result.m283exceptionOrNullimpl(result);
                            ...
                            $this$resumeWithStackTrace$iv.resumeWith(Result.m280constructorimpl(ResultKt.createFailure(th)));
                            return;
                        }
            ...
            if (result$iv == SelectKt.access$getUNDECIDED$p()) {
                ...
                if (result$iv == update$iv2) {
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_result$FU, this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.access$getRESUMED$p())) {
                        ...
                        Result.Companion companion = Result.INSTANCE;
                        intercepted.resumeWith(Result.m280constructorimpl(ResultKt.createFailure(exception)));
                        return;
                    }
                } else {
                    throw new IllegalStateException("Already resumed");
    ...
    public final void handleBuilderException(Throwable e) {
        if (trySelect()) {
            Result.Companion companion = Result.INSTANCE;
            resumeWith(Result.m280constructorimpl(ResultKt.createFailure(e)));
    ...
    public final void doAfterSelect() {
        ...
        for (LockFreeLinkedListNode cur$iv = (LockFreeLinkedListNode) this_$iv.getNext(); !Intrinsics.areEqual(cur$iv, this_$iv); cur$iv = cur$iv.getNextNode()) {
            if (cur$iv instanceof DisposeNode) {
                DisposeNode it2 = (DisposeNode) cur$iv;
                it2.handle.dispose();
            }
        }
    }
    ...
    /* JADX WARN: Code restructure failed: missing block: B:41:0x0035, code lost:            doAfterSelect();     */
    /* JADX WARN: Code restructure failed: missing block: B:42:0x003a, code lost:            return kotlinx.coroutines.CancellableContinuationImplKt.RESUME_TOKEN;     */
    @Override // kotlinx.coroutines.selects.SelectInstance
    /*
    ...
    */
    ...
    public static final class AtomicSelectOp extends AtomicOp<Object> {
        ...
        public Object prepare(Object affected) {
            Object it2;
            if (affected == null && (it2 = prepareSelectOp()) != null) {
                return it2;
            }
            try {
