...
public class JobSupport implements Job, ChildJob, ParentJob, SelectClause0 {
    ...
    private final Throwable getFinalRootCause(Finishing state, List<? extends Throwable> exceptions) {
        ...
        if (first instanceof TimeoutCancellationException) {
            ...
            while (true) {
                ...
                if (((it4 == first || !(it4 instanceof TimeoutCancellationException)) ? null : 1) != null) {
                    obj = element$iv2;
                    break;
                }
            }
    ...
    private final void completeStateFinalization(Incomplete state, Object update) {
        ChildHandle it2 = getParentHandle$kotlinx_coroutines_core();
        if (it2 != null) {
            it2.dispose();
            setParentHandle$kotlinx_coroutines_core(NonDisposableHandle.INSTANCE);
        }
    ...
    private final /* synthetic */ <T extends JobNode> void notifyHandlers(NodeList list, Throwable cause) {
        ...
        if (th2 != null) {
            Throwable it2 = th2;
            handleOnCompletionException$kotlinx_coroutines_core(it2);
        }
    }
    ...
    protected final boolean getCompletionCauseHandled() {
        Object it2 = getState$kotlinx_coroutines_core();
        return (it2 instanceof CompletedExceptionally) && ((CompletedExceptionally) it2).getHandled();
    }
    ...
    private final JobNode makeNode(Function1<? super Throwable, Unit> handler, boolean onCancelling) {
        ...
        if (onCancelling) {
            ...
            if (node != null) {
                JobNode it2 = node;
                if (DebugKt.getASSERTIONS_ENABLED() && !(!(it2 instanceof JobCancellingNode))) {
                    throw new AssertionError();
                }
            } else {
    ...
    private final Object makeCancelling(Object cause) {
        ...
        while (true) {
            ...
            if (state instanceof Finishing) {
                ...
                if (causeException == null) {
                    Throwable it3 = createCauseException(cause);
                    th = it3;
                    causeException = it3;
                } else {
                    th = causeException;
    ...
    private final Object tryMakeCompletingSlowPath(Incomplete state, Object proposedUpdate) {
        ...
        synchronized (finishing) {
            ...
            CompletedExceptionally it2 = proposedUpdate instanceof CompletedExceptionally ? (CompletedExceptionally) proposedUpdate : null;
            if (it2 != null) {
                finishing.addExceptionLocked(it2.cause);
            }
            notifyRootCause.element = Boolean.valueOf(wasCancelling ? false : true).booleanValue() ? finishing.getRootCause() : 0;
            Unit unit = Unit.INSTANCE;
            Throwable it3 = (Throwable) notifyRootCause.element;
            if (it3 != null) {
                notifyCancelling(list, it3);
            }
    ...
    public static final class AwaitContinuation<T> extends CancellableContinuationImpl<T> {
        ...
        public Throwable getContinuationCancellationCause(Job parent) {
            ...
            if (!(state instanceof Finishing) || (it2 = ((Finishing) state).getRootCause()) == null) {
