...
public final class HandlerContext extends HandlerDispatcher implements Delay {
    ...
    private HandlerContext(Handler handler, String name, boolean invokeImmediately) {
        ...
        HandlerContext it2 = this._immediate;
        if (it2 == null) {
            it2 = new HandlerContext(this.handler, this.name, true);
            this._immediate = it2;
        }
        this.immediate = it2;
    }
    ...
    public void mo1833scheduleResumeAfterDelay(long timeMillis, final CancellableContinuation<? super Unit> continuation) {
        ...
        if (this.handler.postDelayed(block, RangesKt.coerceAtMost(timeMillis, DurationKt.MAX_MILLIS))) {
            return new DisposableHandle() { // from class: kotlinx.coroutines.android.HandlerContext$$ExternalSyntheticLambda0
                ...
                public final void dispose() {
                    HandlerContext.m1782invokeOnTimeout$lambda3(HandlerContext.this, block);
                }
            };
        }
        cancelOnRejection(context, block);
    ...
    public static final void m1782invokeOnTimeout$lambda3(HandlerContext this$0, Runnable $block) {
