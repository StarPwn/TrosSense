...
public final class StackTraceRecoveryKt {
    ...
    static {
        ...
        try {
            ...
            obj = Result.m280constructorimpl(Class.forName(baseContinuationImplClass).getCanonicalName());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            obj = Result.m280constructorimpl(ResultKt.createFailure(th));
        }
        ...
        try {
            ...
            obj2 = Result.m280constructorimpl(Class.forName(stackTraceRecoveryClass).getCanonicalName());
        } catch (Throwable th2) {
            Result.Companion companion4 = Result.INSTANCE;
            obj2 = Result.m280constructorimpl(ResultKt.createFailure(th2));
        }
        if (Result.m283exceptionOrNullimpl(obj2) == null) {
            obj3 = obj2;
        }
    ...
    private static final <E extends Throwable> E createFinalException(E e, E e2, ArrayDeque<StackTraceElement> arrayDeque) {
        ...
        while (it2.hasNext()) {
            int index = i;
            i++;
            StackTraceElement element = it2.next();
    ...
    private static final void mergeRecoveredTraces(StackTraceElement[] recoveredStacktrace, ArrayDeque<StackTraceElement> arrayDeque) {
        ...
        while (true) {
            if (index$iv < length) {
                StackTraceElement it2 = recoveredStacktrace[index$iv];
                if (isArtificial(it2)) {
                    break;
                } else {
                    index$iv++;
    ...
    private static final ArrayDeque<StackTraceElement> createStackTrace(CoroutineStackFrame continuation) {
        ...
        StackTraceElement it2 = continuation.getStackTraceElement();
        if (it2 != null) {
            stack.add(it2);
        }
        ...
        while (true) {
            ...
            last = callerFrame;
            StackTraceElement it3 = last.getStackTraceElement();
            if (it3 != null) {
                stack.add(it3);
            }
        }
    ...
    private static final int frameIndex(StackTraceElement[] $this$frameIndex, String methodName) {
        ...
        for (int index$iv = 0; index$iv < length; index$iv++) {
            StackTraceElement it2 = $this$frameIndex[index$iv];
            if (Intrinsics.areEqual(methodName, it2.getClassName())) {
                return index$iv;
            }
        }
        return -1;
